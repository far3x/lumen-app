...
from app.services.valuation import advanced_valuation_service
...
def process_contribution(user_id: int, codebase: str):
    ...
    try:
        ...
        print(f"Processing contribution for user {user_id}.")

        valuation_result = advanced_valuation_service.calculate(db, codebase)
        base_reward = valuation_result.get("final_reward", 0.0)
        
        print(f"Valuation results: {valuation_result.get('valuation_details', {})}")

        if base_reward > 0:
            total_reward_given = crud.apply_reward_to_user(db, user, base_reward)
            avg_complexity = valuation_result.get("valuation_details", {}).get("avg_complexity", 0.0)
