...
function render() {
    ...
    if (formState === 'success') {
        container.innerHTML = `
            ...
            <h1 class="text-3xl font-bold text-text-main">Password Reset!</h1>
            <p class="text-text-secondary mt-3">Your password has been successfully updated. You can now log in with your new password.</p>
            <button id="go-to-login" class="mt-8 w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-accent-primary hover:bg-red-700 transition-colors">
                Go to Login
            </button>
        `;
    ...
    container.innerHTML = `
        <h1 class="text-3xl font-bold text-text-main">Set a New Password</h1>
        ...
        <div class="min-h-screen flex items-center justify-center p-6">
            <div class="container mx-auto grid lg:grid-cols-2 gap-12 items-center">
                <div class="w-full max-w-md mx-auto text-center">
                    <div id="reset-password-container" class="bg-surface p-8 rounded-xl border border-primary shadow-2xl shadow-black/5">
                    </div>
                </div>
